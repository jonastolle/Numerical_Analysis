...
>$$\begin{split}&\limsup_{k\to\infty}\frac{|x_{k+1}-x|}{|x_k-x|^p}=\limsup_{k\to\infty}\frac{|x_{k+1}-x|}{|x_k-x|^{p^*}}|x_k-x|^{p^*-p}\\\le&\limsup_{k\to\infty}\frac{|x_{k+1}-x|}{|x_k-x|^{p^*}}\limsup_{k\to\infty}|x_k-x|^{p^*-p}=C(p^*)\cdot 0=0.\end{split}$$
...
### Newton's method
<!--stackedit_data:
eyJoaXN0b3J5IjpbOTEwNDYyMDQzLDEyNDc5NzI2MTIsLTE5OD
