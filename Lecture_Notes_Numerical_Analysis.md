...
where $a_s=1$ and the coefficients $a_0,\ldots, a_{s-1}$ and $b_0,\ldots,b_s$ determine the method.
...
$$|d_{k+1}|\le e^{khL}\u|d_0|+\frac{e^{khL}-1}{Lh}\delta$$
...
2. It is not guaranteed that $\{\bar{w}_N\}_{N\in\mathbb{N}}$ converges to $w_\ast$ unless $w_\ast$ is the unique minimizer (e.g. if $f$ is so-called *strictly convex* (i.e., the inequality in the definition of convexity is strict for $\lambda\in (0,1)$).
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE5MzEyMDMwMDcsLTgyNzcyNzA2MSwxNj
